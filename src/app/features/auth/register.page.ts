  ...
  template: `
    <section class="min-h-[calc(100dvh-var(--header-height))] grid place-items-center px-5 py-10">
      <div class="w-full max-w-[520px] forge-surface forge-surface--loose">
        <h1 class="text-2xl font-black">{{ 'auth.register.title' | transloco: {} : 'auth' }}</h1>
        <p class="mt-2 text-white/80">{{ 'auth.register.subtitle' | transloco: {} : 'auth' }}</p>

        <form class="mt-5 grid gap-3" [formGroup]="form" (ngSubmit)="onSubmit()">
          <mat-form-field appearance="outline" class="w-full">
            <mat-label>{{ 'auth.register.name' | transloco: {} : 'auth' }}</mat-label>
            <input matInput formControlName="name" autocomplete="name" />
          </mat-form-field>

          <mat-form-field appearance="outline" class="w-full">
            <mat-label>{{ 'auth.register.email' | transloco: {} : 'auth' }}</mat-label>
            <input matInput formControlName="email" autocomplete="email" />
          </mat-form-field>

          <mat-form-field appearance="outline" class="w-full">
            <mat-label>{{ 'auth.register.password' | transloco: {} : 'auth' }}</mat-label>
            <input
              matInput
            ...
            <button
              ...
              [attr.aria-label]="
                passwordVisible()
                  ? ('auth.register.hidePassword' | transloco: {} : 'auth')
                  : ('auth.register.showPassword' | transloco: {} : 'auth')
              "
            >
